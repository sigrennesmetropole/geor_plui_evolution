...
const createWebpackConfig = ({prod = false} = {}) => {
    const configuration = buildConfig(
        {
            'plui-evolution-test-app': path.join(__dirname, "..", "js", "app")
        },
        themeEntries,
        {
            base: __dirname,
        ...
        [
            new CopyPlugin( [
                    { from: path.resolve(__dirname, "..", 'translations', 'data.fr-FR.json'), to: 'data.fr-FR.json'},
                    { from: path.resolve(__dirname, "..", 'assets', 'index.json'), to: 'index.json'}
                ])
            ,
            new ZipPlugin({
                filename: 'plui-evolution.zip',
                include: [
                    "index.json",
                    "extensions/plui-evolution.js",
                    "data.fr-FR.json"
                    
                ],
                pathMapper: assetPath => {
                	console.log("======>pathMapper:" + assetPath);
                    if(assetPath.startsWith('data')) {
                        return path.join("translations/", assetPath);
                    }
